...
def model_exe_funcs(output,y,losstype,learning_rate):
    ...
    return f_prediction, f_actual, f_cost, optimizer, init
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            while L_seq < L_batch:
                ...
                i += 1
                if i % 20 == 0:
                    print("Epoch %i, after %i windows trainded %i samples, loss = %.3f, accuracy = %.2f, f1-score: %.2f" % (epoch, i, processed_sz,loss,accuracy,f1))
...
def validate_models(sess, 
                    valid_x, valid_y, #data
                    x, y, states_in, keep_prob, #placeholders
                    f_prediction, f_actual, f_cost, init, #modelling functions
                    n_layers, nodes, #for setting initial state
                    valid_window): 
    ...
    for i in range(np.ceil(valid_x.shape[0]/valid_window).astype(np.int)):
...
#define model execution parameters
f_prediction, f_actual, f_cost, optimizer, init = model_exe_funcs(output,y,losstype,learning_rate)

#train_models(train_x, train_y, dropout, n_layers, nodes, range_B, n_epochs, seed, losstype = 'logloss')

#train the model
sess, accuracy, f1, seed = train_models(train_x, train_y, dropout, #data
                 x, y, states_in, keep_prob, #placeholders
                 state, #from forward propagation
                 f_prediction, f_actual, f_cost, optimizer, init, #modelling functions
                 n_layers, nodes, #for setting initial state
...
#validate the model
accuracy, f1 = validate_models(sess, 
                               valid_x, valid_y, #data
                               x, y, states_in, keep_prob, #placeholders
                               f_prediction, f_actual, f_cost, init, #modelling functions
                               n_layers, nodes, #for setting initial state
