...
dim = len(train_x[0]) #number of features features
valid_len = len(valid_x) #number of samples in the validation set
test_len = len(test_x) #number of samples in the test set

train_y = np.array(train_y)

#####################################################
# We can change valid and test batch size to increase the speed of training process.
# In test and valid process, they have same window length.
test_win = 5000
test_bt = 1
...
rnn_tuple_state = tuple(
		 [tf.nn.rnn_cell.LSTMStateTuple(states[layer][0],states[layer][1])
...
def lstm_cell(dropout_keep_prob):
	...
	return tf.contrib.rnn.DropoutWrapper(tf.nn.rnn_cell.LSTMCell(rnn_size), output_keep_prob=dropout_keep_prob, state_keep_prob=dropout_keep_prob)
...
def recurrent_neural_network (x,rnn_tuple_state,dropout_keep_prob):
	...
	outputs, states = tf.nn.dynamic_rnn(stacked_lstm, x, initial_state=rnn_tuple_state, dtype=tf.float32)
...
def making_training_set(train_x, train_y, batch_size=100):
	...
	for i in range(batch_size):
		...
		y_train3D[i,:,:] = train_y[idx_start:idx_end, :]# if train_y is a pandas dataframe
	...
	with tf.Session() as sess:
		...
		for epoch in range(nm_epochs):
			...
			while pos_end < train_len:
				...
				_, c, predict_out, state_ini, train_acc = sess.run([optimizer, cost, prediction, state, accuracy_train], feed_dict={x: inputs, y: targets, states: state_ini, dropout_keep_prob: keep_rate})
				epoch_loss += c
				...
				i += 1
				if i % 20 == 0:
					print("Pred. sum %.3f Traing %06i, Train Accuracy = %.2f, Train Loss = %.3f" % (np.sum(predict_out), pos_end*batch_size, train_acc, c))
